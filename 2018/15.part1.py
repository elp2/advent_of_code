...
def find_all_paths(x, y, fighters, board):
    ...
    # Find paths to all spaces around enemies
    # Find shortest path to enemy
    # return that.

class Fighter:
    def __init__(self, team, x, y, hp=200):
        self.team = team
        self.x = x
        self.y = y
        self.hp = hp
    ...
    def attack_around(self, fighters, board):
        sorted_enemies = self.sorted_enemies(fighters)
        enemies = []
        for a in around(self.x, self.y, board):
    ...
    def action(self, fighters, board):
        # Attack adjacent.
        if self.attack_around(fighters, board):
            return
        ...
        adjacents = set()
        for e in enemies:
            for ax, ay in around(e.x, e.y, board):
                adjacents.add((ax, ay))
        ...
        self.attack_around(fighters, board)

CHALLENGE_DAY = "15"
REAL = open(CHALLENGE_DAY + ".txt").read()
SAMPLE = open(CHALLENGE_DAY + ".sample").read()
SAMPLE_EXPECTED = 27730
# SAMPLE_EXPECTED = 
...
def find_fighters(parsed):
    fighters = []
    for y in range(len(parsed)):
        for x in range(len(parsed[y])):
            here = parsed[y][x]
            if here in "GE":
                fighters.append(Fighter(here, x, y))
                parsed[y][x] = "."
    return fighters
...
def run_round(fighters, board):
    # units take their turns within a round is the reading order of their starting positions in that round
...
def find_winners(fighters, round):
...
def solve(raw):
    ...
    fighters = find_fighters(parsed)

    round = 0
    while True:
        round_finished = run_round(fighters, parsed)
        if round_finished:
            return find_winners(fighters, round)
